...
import { IMAGE_SIZE } from './layout.ts';
...
describe('image round-trip', () => {
  ...
  it('round-trips a few channels byte-for-byte through parse(serialize)', () => {
    ...
    const bytes = serializeImage(img);
    expect(bytes.length).toBe(IMAGE_SIZE);

    const reparsed = parseImage(bytes);
  ...
  it('produces byte-stable output on a no-op edit cycle', () => {
    ...
    const first = serializeImage(img);
    const reparsed = parseImage(first);
    const second = serializeImage(reparsed);
